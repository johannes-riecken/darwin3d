...
double FASTATAN (double z)
{
    // All approximations valid for |z| <= 1.  To compute ATAN(z)
    // for z > 1, use ATAN(z) = PI/2 - ATAN(1/z).  For z < -1, use
    // ATAN(z) = -PI/2 - ATAN(1/z).

    // Speed-ups based on executions on an Intel Pentium 90Mhz chip
    // running under Microsoft Visual C++ 4.0 with optimization set
    // for "maximum speed".  Comparison is made to the run-time
    // library routine atan(double).
...
#if 0
    // Polynomial approximation of degree 16, Q(z)
    // |ATAN(z)-Q(z)| <= 2e-08
    // Speed-up is 1.54
